...
const Terminal = () => {
  function reducer(state, action) {
    switch (action.type) {
      ...
      case "EXECUTE_COMMAND":
        return { ...state, terminalHistory: [...state.terminalHistory, action.payload], commandInput: "" }
  ...
  const handleInputChange = (e) => {
    ...
    let commandMatches = Object.keys(availableCommands).includes(e.target.value.trim())
    dispatch({ type: "HIGHLIGHT_COMMAND", payload: commandMatches })
  }
  const executeCommand = () => {
    dispatch({ type: "EXECUTE_COMMAND", payload: { command: inputRef.current.value, commandOutput: findCommand(inputRef.current.value).output() } })
  ...
  const findCommand = (command) => {
    ...
    if (availableCommands.hasOwnProperty(command)) {
      ...
      foundCommand = { output() { return <p>unknown command : {command}</p> } }
    }
    return foundCommand
  }
  ...
  const showLastCommand = ()=>{
    ...
    let commandMatches = Object.keys(availableCommands).includes(lastCommand.command)
    dispatch({ type: "HIGHLIGHT_COMMAND", payload: commandMatches })
  }
  ...
  return (
    <div className="bg-neutral-950 h-screen text-foreground font-mono text-sm px-2">
      ...
      <div id="input" className="flex gap-x-2">
        ...
        <input spellCheck="false" value={state.commandInput} onChange={handleInputChange} ref={inputRef} type="text" name="command input" className={`outline-0 w-full inline  ${state.commandHighlight ? "text-blue-600" :"text-red-600"}`} />
      </div>
...
function Welcom() {
  return (
    <>
      <p>Type <span className="text-lime-500">help</span> to see available commands</p>
    </>)
}